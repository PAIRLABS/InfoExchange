...
with open('cnt_friend', 'w') as cnt_cmd:
    statement = 'db._useDatabase("test")\n'
    cnt_cmd.write(statement)
    for name in name_list:
        statement = 'var Cnt = new Set(db._query(aql`for per in person filter per.name=="{0}" for pap in 1..1 outbound per author_of for per2 in 1..1 inbound pap author_of return per2.name`).toArray())\n'.format(name)
        statement +='print("{0}",":",Cnt.size.toString())\n\n'.format(name)
        cnt_cmd.write(statement)
    cnt_cmd.close()
