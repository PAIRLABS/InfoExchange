...
################################################
    for name in name_list:
        ...
        if problem_dict2.__contains__(name):
            exprs = exprs = "Composite(AA.AuId="+ problem_dict2[name] + ")"
        elif problem_dict3.__contains__(name):
        ...
        try:
            for ENTITY in json_data['entities']:  #save paper
                ...
                for PERSON in ENTITY['AA']:
                    ...
                    if (author==problem_dict3.get(name) or author_id==problem_dict2.get(name)):
                        print(name + " : " + author + " , " + str(author_id))
                        ...
                        statement_2+='    db._update(person_id, {1}English_name:"{0}",MAG_id: mids{2});\n'.format(author,LQ,RQ)
                    else:
                        statement_2+='    if(person_id = db._query(`for per in person filter per.name=="{0}" return per._id`).toArray()[0]) {1}\n'.format(author,LQ)
                        statement_2+='        mids = mids.concat(db._document(person_id)["MAG_id"]) \n'
                        statement_2+='        mids.push("{0}")\n'.format(author_id)
                        statement_2+='        mids = Array.from(new Set(mids.filter( function(e){0} return e {1} )))\n'.format(LQ,RQ)
    ...
    print('emp_cnt : ' + str(empty_cnt) + " / " + str(len(name_list)))
