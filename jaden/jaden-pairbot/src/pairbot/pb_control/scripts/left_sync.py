...
# T_LEFT_BASE_WHEELS_2nd = "joint_left_base_wheel_2nd_{0}_controller/command"
...
# T_LF_FMW_2nd = 'joint_left_front_flipper_middle_wheel_2nd_{0}_controller/command'
# T_LR_FMW_2nd = 'joint_left_rear_flipper_middle_wheel_2nd_{0}_controller/command'
...
#     pubs.extend([ rospy.Publisher(T_LEFT_BASE_WHEELS_2nd.format(i+1), Float64, queue_size=10) for i in range(11)])
    ...
    pubs.extend([ rospy.Publisher(T_LF_FMW.format(i+1), Float64, queue_size=10) for i in range(8)])
    pubs.extend([ rospy.Publisher(T_LR_FMW.format(i+1), Float64, queue_size=10) for i in range(8)])
#     pubs.extend([ rospy.Publisher(T_LF_FMW_2nd.format(i+1), Float64, queue_size=10) for i in range(6)])
#     pubs.extend([ rospy.Publisher(T_LR_FMW_2nd.format(i+1), Float64, queue_size=10) for i in range(6)])
