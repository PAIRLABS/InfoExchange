...
void StairDetection::test()
{
	...
	while(ros::ok())
	{
		...
		Depth_Arr();
		a=(int)depthMD(width/2,height/2);
		printf("data=%d\n",a);
...
void StairDetection::SD_Start()
{
	...
	while(ros::ok())
	{
		...
		depth_find_5(5,&mid_x5,&mid_y5,&i5,&j5,&x5,&sw5);
		...
		d21_temp=d21_temp*2;//increase sensitivity
		...
		// std::cout<<"d21 at ("<<mid_x-150<<" , "<<mid_y+20<<")= "<<depthMD(mid_x-150,mid_y+20)<<"\n";
		// std::cout<<"d21 at ("<<mid_x-150<<" , "<<mid_y+25<<")= "<<depthMD(mid_x-150,mid_y+25)<<"\n";
		// std::cout<<"d21 at ("<<mid_x-150<<" , "<<mid_y+30<<")= "<<depthMD(mid_x-150,mid_y+30)<<"\n";
		// std::cout<<"d21 at ("<<mid_x+150<<" , "<<mid_y+20<<")= "<<depthMD(mid_x+150,mid_y+20)<<"\n";
		// std::cout<<"d21 at ("<<mid_x+150<<" , "<<mid_y+25<<")= "<<depthMD(mid_x+150,mid_y+25)<<"\n";
		// std::cout<<"d21 at ("<<mid_x+150<<" , "<<mid_y+30<<")= "<<depthMD(mid_x+150,mid_y+30)<<"\n";
		// std::cout<<"midff87\n";
		...
		{
			d4.data=(int)(((depthMD(x4+20,j4+30)+depthMD(x4+20,j4+20)+depthMD(x4+20,j4+40))/3-(depthMD(i4-20,sw4+30)+depthMD(i4-20,sw4+20)+depthMD(i4-20,sw4+40))/3));
		}
		// mode 5~7
		...
		d.data=(int)depthMD(mid_x5,mid_y5+5);
		d51=(int)(depthMD(x5+30,j5+30)+depthMD(x5+30,j5+20))/2;
		d52=(int)(depthMD(i5-30,sw5+30)+depthMD(i5-30,sw5+20))/2;
		d71=(int)(depthMD(x7+20,j7+15)+depthMD(x7+20,j7+20)+depthMD(x7+20,j7+10))/3;
		d72=(int)(depthMD(i7-20,sw7+15)+depthMD(i7-20,sw7+20)+depthMD(i7-20,sw7+10))/3;
		...
		//cv::circle(img1, cv::Point(mid_x2,mid_y2),1 ,cv::Scalar(0,255,0),  2,  8,  0);
		...
		if (depth_imgshow)
			cv::imshow(OPENCV_WINDOW2,img2);//show result
...
void StairDetection::imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  int w=width;
  int i,j;
  //printf("Calbak, lenofmsg=%lu\n",sizeof(msg->data));

  for(i=0;i<h;i++)  //Unit m multiply 100 convert to cm
  {
    for(j=0;j<w;j++)
    {
        float f;
		...
		if (f!=f || f <0.3){
			dis[j][i] = 0.3*100;
			// if (i==100)
			// printf("nan set to 0.3, f=%f\n",f);
		}
       	else{
			dis[j][i] = f*100;
			if (isnan(dis[j][i]))
				printf("dis=%f",dis[j][i]);
		}
   }
  }
  //printf("Calbak, lenofdis=%lu\n",sizeof(dis));
...
void StairDetection::depth_find(int dc,int *mid_x2,int *mid_y2,int *midx,int *midy,int *di,int *dj,int *dx,int *dsw)
{
    ...
    if(dc==21)	/*---For Mode 21---*/
	{
		...
		{
			for(int jj=1;jj<height;jj++)
			{		//*** check if y's depth changes a lot to confirm if it is stair 
				if((abs(depthMD(ii,jj-1)-depthMD(ii,jj))>t1)&&(abs(depthMD(ii,jj-1)-depthMD(ii,jj))<t2)&&(depthMD(ii,jj-1)!=0)&&(depthMD(ii,jj)!=0))
				{
					c=1;
					ptr2[ii][jj]=c;
